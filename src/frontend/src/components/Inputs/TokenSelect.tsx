...
interface Props {
  ...
  setBigger: (val: boolean) => void;
}
interface State {
  show: boolean;
}
class TokenSelect extends Component<Props, State> {
...
class TokenSelectOptionItem extends Component<IProps, IState> {
  ...
  componentDidMount() {
    ...
    if (this.props.canisterId) {
      getDTokenBalance(this.props.canisterId, this.props.decimals).then(
        (balance) => {
